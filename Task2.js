...
console.log(getSkills(workers));

function getBalance(workers) {
    totalBalance = 0;

    skills = []
    workers.forEach(worker => {
        if (worker.friends.length == 2 && worker.age < 30) {
            totalBalance += worker.balance;
        }
    });
...
console.log(getBalance(workers));